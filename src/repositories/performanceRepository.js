...
export const performanceRepository = {
    ...
    artistUnavailability: async(artist_id, start_time, end_time, date, current_performance_id = null) => {
        ...
        const text = 'SELECT EXISTS (SELECT 1 FROM performance WHERE artist_id = $1 AND date = $2 AND start_time < $3 AND end_time > $4 AND ($5::int IS NULL OR performance_id != $5)) AS is_unavailable;';
        ...
        // Se current_performance_id for nulo (true) (quando estamos criando uma nova performance), a condição ($5::int IS NULL) será verdadeira. Isso significa que a segunda parte da condição (performance_id != $5) não precisa ser avaliada porque o operador 'OR' já encontrou uma condição verdadeira. Portanto, nenhuma performance será excluída da verificação de conflitos baseada em ID,e a consulta SQL irá procurar por qualquer performance que possa conflitar com os horários propostos da nova performance.
        // Se current_performance_id não for nulo, contiver um id, (false) (quando estamos atualizando uma performance já existente), o SQL precisa avaliar a segunda parte da condição (performance_id != $5) porque a primeira parte da condição é falsa. Neste caso:
            // - Se performance_id != $5 for verdadeiro, isso significa que a performance que está sendo examinada (em cada iteração da consulta) é diferente da performance que está sendo atualizada. Assim, essas performances são incluídas na verificação de conflitos.
            // - Se performance_id != $5 for falso (ou seja, id é igual a $5), isso significa que estamos olhando para a própria performance que está sendo atualizada. Essa performance será excluída da verificação de conflitos para evitar falsos positivos, pois não queremos que a performance detecte a si mesma como um conflito.
        // Resumindo:
        ...
        //O que garante a exclusão da própria  da própria performance que está sendo atualizada da busca por conflitos é a condição performance_id != $5, ele atua como um filtro. A condição performance_id != $5 significa "considere apenas aquelas performances cujo id é diferente do id que estamos atualizando". Ao avaliar essa condição, o SQL verifica cada performance na base de dados e compara seu id com o id fornecido ($5). Se o id da performance na base de dados for diferente de $5, a performance é considerada na verificação de conflitos. Se for o mesmo, essa performance (a que está sendo atualizada) é excluída da verificação.
        //Em resumo, a parte $5::int IS NULL da condição é crucial para lidar com situações de criação de novas performances, garantindo que todas as performances sejam verificadas por conflitos, enquanto a parte performance_id != $5 é essencial durante a atualização de performances, permitindo que a própria performance sendo atualizada seja excluída da verificação de conflitos. 
    ...
    stageUnavailability: async(stage_id, start_time, end_time, date, current_performance_id = null) => {
        const text = 'SELECT EXISTS (SELECT 1 FROM performance WHERE stage_id = $1 AND date = $2 AND start_time < $3 AND end_time > $4 AND ($5::int IS NULL OR performance_id != $5)) AS is_unavailable;';
